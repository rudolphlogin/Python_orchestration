...
while(dateIter <= endAt):
	...
	cmd = 'rm '+config.get('doubleclickSearch', 'temp_folder_for_hist_files')+'*conversion*'
	print cmd
	...
	for advertiser in advertisers:
		...
		body['reportType'] = config.get('DSconversion', 'reportType')
		body['columns'] = ast.literal_eval(config.get('DSconversion', 'columns'))
		...
		conversion_extract_name,  conversion_extract_file= download_file(url, headers, advertiser+'_conversion'+adl_folder_date_fmt+'.csv')
		print conversion_extract_name
		print conversion_extract_file
	
	#########################################################################
	
	#ADL Copy starts here
	print 'ADL folder creation starts'
	hdfsCmd = 'hadoop fs -mkdir '+config.get('doubleclickSearch', 'adl_end_point_his')+'conversion/'+adl_folder_date_fmt
	print hdfsCmd 
	...
	# hdfsCmd = 'hadoop fs -put '+conversion_extract_name+' '+config.get('doubleclickSearch', 'adl_end_point_his')+'conversion/'+adl_folder_date_fmt+"/"+conversion_extract_file
	hdfsCmd = 'hadoop fs -put ' +config.get('doubleclickSearch', 'temp_folder_for_hist_files')+ '*_conversion*.csv ' +config.get('doubleclickSearch', 'adl_end_point_his')+'conversion/'+adl_folder_date_fmt+"/"
	...
	hdfsCmd = 'hadoop fs -mkdir '+config.get('doubleclickSearch', 'archive_adl_end_point_his')+'conversion/'+adl_folder_date_fmt
	print hdfsCmd
	output = commands.getstatusoutput(hdfsCmd)
	status = output[0]
	print status
	...
	hdfsCmd = 'hadoop fs -cp '+config.get('doubleclickSearch', 'adl_end_point_his')+'conversion/'+adl_folder_date_fmt+"/*"+' '+config.get('doubleclickSearch', 'archive_adl_end_point_his')+'conversion/'+adl_folder_date_fmt+"/"
	print hdfsCmd
	output = commands.getstatusoutput(hdfsCmd)
	status = output[0]
	print status
	...
	cmd = "hive -e \"alter table cdp_raw_data.rw_ds_conversion drop partition (file_dt > '0');\""
	print cmd
	commands.getstatusoutput(cmd)
	
	print 'Adding new partition to raw table'
	cmd = "hive -e \"alter table cdp_raw_data.rw_ds_conversion add partition (file_dt="+adl_folder_date_fmt+") location 'adl://cdpuat.azuredatalakestore.net/cdp_raw_store/doubleclick_search/conversion/"+adl_folder_date_fmt+"';\"";
	print cmd
	commands.getstatusoutput(cmd)
	
	time.sleep(6)
	dateIter = dateIter + timedelta(days=1)	
print 'End of DS conversion data pull script'
