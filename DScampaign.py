...
while(dateIter <= endAt):
	...
	cmd = 'rm '+config.get('doubleclickSearch', 'temp_folder_for_hist_files')+'*campaign*'
	print cmd
	...
	for advertiser in advertisers:
		...
		body['reportType'] = config.get('DScampaign', 'reportType')
		body['columns'] = ast.literal_eval(config.get('DScampaign', 'columns'))
		...
		campaign_extract_name,  campaign_extract_file= download_file(url, headers, advertiser+'_campaign'+adl_folder_date_fmt+'.csv')
		print campaign_extract_name
		print campaign_extract_file
	
	#########################################################################
	
	#ADL Copy starts here
	print 'ADL folder creation starts'
	hdfsCmd = 'hadoop fs -mkdir '+config.get('doubleclickSearch', 'adl_end_point_his')+'campaign/'+adl_folder_date_fmt
	print hdfsCmd 
	...
	# hdfsCmd = 'hadoop fs -put '+campaign_extract_name+' '+config.get('doubleclickSearch', 'adl_end_point_his')+'campaign/'+adl_folder_date_fmt+"/"+campaign_extract_file
	hdfsCmd = 'hadoop fs -put ' +config.get('doubleclickSearch', 'temp_folder_for_hist_files')+ '*_campaign*.csv ' +config.get('doubleclickSearch', 'adl_end_point_his')+'campaign/'+adl_folder_date_fmt+"/"
	...
	hdfsCmd = 'hadoop fs -mkdir '+config.get('doubleclickSearch', 'archive_adl_end_point_his')+'campaign/'+adl_folder_date_fmt
	print hdfsCmd
	output = commands.getstatusoutput(hdfsCmd)
	status = output[0]
	print status
	...
	hdfsCmd = 'hadoop fs -cp '+config.get('doubleclickSearch', 'adl_end_point_his')+'campaign/'+adl_folder_date_fmt+"/*"+' '+config.get('doubleclickSearch', 'archive_adl_end_point_his')+'campaign/'+adl_folder_date_fmt+"/"
	print hdfsCmd
	output = commands.getstatusoutput(hdfsCmd)
	status = output[0]
	print status
	...
	cmd = "hive -e \"alter table cdp_raw_data.rw_ds_campaign drop partition (file_dt > '0');\""
	print cmd
	commands.getstatusoutput(cmd)
	
	print 'Adding new partition to raw table'
	cmd = "hive -e \"alter table cdp_raw_data.rw_ds_campaign add partition (file_dt="+adl_folder_date_fmt+") location 'adl://cdpuat.azuredatalakestore.net/cdp_raw_store/doubleclick_search/campaign/"+adl_folder_date_fmt+"';\"";
	print cmd
	commands.getstatusoutput(cmd)
...
print 'End of DS campaign data pull script'